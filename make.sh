...
if [ -f "${OUTPUT_DIR}/alice-sms-webhook_mbedtls" ]; then 
    ...
    # 重命名文件
    mv "${OUTPUT_DIR}/alice-sms-webhook_mbedtls" "${OUTPUT_DIR}/alice-pusher-bot-zxic"
    
    echo "可执行文件已重命名为: ${OUTPUT_DIR}/alice-pusher-bot-zxic" 
    echo "优化后文件大小: $(du -h ${OUTPUT_DIR}/alice-pusher-bot-zxic | cut -f1)" 
else 
    echo "编译失败：未能生成可执行文件" 
    exit 1 
fi
