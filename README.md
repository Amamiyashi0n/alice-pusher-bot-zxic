...
3. 编译成功后生成主程序 `output/alice-pusher-bot`、独立 BearSSL 动态库
   ...
   ```
   由持久化入口开机拉起时会同时启动 WebUI 和短信监控服务（需已配置有效推送任务），并跳过重复安装；需要移除时在 WebUI 中点击“卸载自启动”。
...
- 手动启动 WebUI 会自动执行持久化安装或升级；由持久化入口启动时会同时拉起短信监控服务（需已配置有效推送任务）且不会重复写入。WebUI 卸载后，本次运行会话内的端口切换或 WebUI 重启不会自动重装，下一次手动运行安装包时才会再次安装。
- WebUI 绑定目标端口遇到占用时，会在目标 Linux 设备上从 `/proc/net/tcp*` 定位监听 PID，先终止旧进程，必要时强制杀死后重试绑定；不会依赖主机 ADB 或本机端口状态。
- 如果 `/mnt/userdata` 可用空间不足以保存当前 payload，WebUI 的“开机自启动”状态会显示“设备不支持”，并禁用安装按钮。
