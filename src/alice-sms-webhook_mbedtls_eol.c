...
void* strace_thread_func(void* arg) {
    ...
    if (child == 0) {
        ...
        // 优化strace参数，只跟踪read系统调用，降低CPU占用
...
#elif STRACE_RESTART_MODE == 0
                // 仅在strace意外停止时重启
                pid_t current_strace_pid = get_strace_pid_from_file();
                if (current_strace_pid > 0) {
                    // 检查strace进程是否仍在运行
                    if (kill(current_strace_pid, 0) != 0) {
                        printf("检测到strace进程已停止，正在重启...\n");
                        // strace进程已停止，需要重启
                        int ztepid = find_zte_mifi_pid();
                        if (ztepid > 0) {
                            pid_t c2 = fork();
                            if (c2 == 0) {
                                char pidstr2[16];
                                snprintf(pidstr2, sizeof(pidstr2), "%d", ztepid);
                                execl(strace_bin_path, "strace", "-f", "-e", "trace=read", "-s", "1024", "-p", pidstr2, "-o", "/tmp/zte_log.txt", (char*)NULL);
                                _exit(127);
                            } else if (c2 > 0) {
                                set_strace_pid_to_file(c2);
...
void extract_and_send_sms_from_log(const char *webhook, const char *headtxt, const char *tailtxt, const char *keyword, const char *number) {
    ...
    char line[2048];
    regex_t reg;
    // 修改正则表达式以匹配实际的read调用格式
    regcomp(&reg, "^[ \t]*read\\([0-9]+, \"\\\\r\\\\n\\+CMT:.*\", [0-9]+\\) = [0-9]+", REG_EXTENDED);
    int line_num = 0;
                        ...
                        if (textlen > 0) {
                            ...
                            if (detection_idx != -1 && detection_window[detection_idx].count >= 3) {
                                ...
                                if (!already_sent) {
                                    ...
                                    char full_msg[1024];
                                    char headtxt_with_number[512] = {0};
